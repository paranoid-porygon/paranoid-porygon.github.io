---
layout: post
title:  "Facebook finally banned me"
summary: "Tried to check my messages and my account was disabled with no explanation and no recourse."
date: 2025-10-22  
tags: [enshittification]
published: true
---

# Facebook finally banned me

On Monday, October 20th, 2025, after having spent most of the weekend offline either at the [No Kings][no-kings] protest or playing Pokemong Legends: Z-A on the Nintendo Switch 1, I attempted to access Facebook to see if I had any messages. I don't have any META apps --or any other social media apps, for that matter-- installed on my phone and only check their sites from a web browser on a cadence of roughly once a day for no more than a few minutes. I keep the account logged in and the site cookie fresh, however this time I was made to manually log in for some reason, which at the time I assumed was because I regularly swap VPN servers and META wanted to verify that my account wasn't compromised. So I logged in, and this was the screen that I was presented with:

![your-account-has-been-disabled][login-screen]{:width="100%"} 

No explanation was given as to why my account was disabled. I clicked the Help Center link provided, and this was what I got:

![help-center][help-center]{:width="100%"}

A few things of note here. First, they claim that if your account is suspended or disabled (the latter of which seems to be the case for me), you'll be informed in an email of such; I received no such email. Second, it claims that someone whose account is suspended would be able to log in and appeal the suspension; I saw no option to appeal, so either the site is buggy or I was disabled and not suspended and those are disttinct from each other. Third, they note that "we may disable your account without suspending it first, such as in cases of severe or time-sensitive violations" and then supply a link to their [Community Standards][fb-community-standards] page which outlines rules that may have been violated; reviewing these list of possible infractions, none of the ones that I likely violated strike me as "severe" or "time-sensitive", and even if they were would still warrant an email explanation.

![community-standards][community-standards-png]{:width="100%"}

The items I have highlighted were the ones I suspected that I might have been dinged for, so I read the details for each:
* [Account Integrity][fb-account-integrity]
* [Authentic Identity Representation][fb-athentic-identity]
* [Cybersecurity][fb-cybersecurity]
* [Inauthentic Behavior][fb-inauthentic-behavior]

The only section of any of these that looked remotely applicable was this, from [Authentic Identity Representation][fb-athentic-identity]:

![authentic-identity][authentic-identity-png]{:width="100%"}

Which, if this is what they used to justify the account lock, has a couple of issues:
* I did recently change the email address that was tied to my account, however that had been changed for several weeks by the time the ban happened and I was still regularly interacting with the same people via Messenger that I had been since I created the account in 2021
* I had updated my bio details a few weeks ago, but META has no way of verifying if those details are correct or incorrect because I don't have my current employer shared publicly anywhere on the web, don't have an active LinkedIn account, or have my employer on any of my credit reports
* I don't believe that I had a "profile location" set
* While I did share and tag myself in images from [thispersondoesnotexist.com](thispersondoesnotexist.com) to throw off facial recognition, my account's profile picture was the default Facebook profile image, and my cover image was blank

These were the only community standards I came close to violating, however in absence of either an email or login alert explaining the justification for my account being disabled, this is entirely speculation. I reached out to what I believe is META's appeal account, appeals@fb.com, asking for an explanation and to have my account reinstated but have not received a response yet. 

I suspect that my regular VPN use, that I changed my login email to a [ProtonMail alias address][proton-alias], and that the relatively sparse details and activity on my profile page may have triggered their anti-bot/abuse safeguards, but I am surprised that the age of the account and that I still regularly used Messenger to communicate with the same people as I did since the account was created didn't indicate otherwise. The absence of an email or message explaining why they decided to disable my account strikes me as unprofessional, even worse that there was no apparent avenue to appeal the decision (I had to hunt down that appeals email address, and I'm not convinced it's even active). I anticipate that if META support gets back to me that they will request government-issued ID to verify the authenticity of my account, at which point I will tell them to go fuck themselves because 1) I am not giving them something that sensitive and 2) they have no way to verify that it's not stolen (it is trivially easy to get a stolen ID on the dark web; I have done this during an investigation).

# I'm not mad, actually I'm laughing

Facebook is a dogshit website whose utility has been on a steady decline for at least 6 years, and its parent company and its leadership are morally bankrupt ghouls that the world would be better off not giving their data to. I primarily used Facebook to hop on Messenger and Marketplace, and there are comparable and even better alternatives for each of those features (Signal and Craigslist). If I can't get my account reinstated, it will be precisely zero skin off my ass. I still have a menagerie of burners that they haven't banned, and more and more people that I care to instant message are starting to use Signal.

I started this personal site/blog with the [explicit declaration](/about#this-site-why) that "the companies and individuals who run these sites have proven themselves to be capricious and arbitrary in moderating the content on their sites", and I didn't expect that so soon after I wrote those words that I'd be given a first-hand object lesson demonstrating them. I'm fortunate that my Facebook page was relatively empty, that I stored the photos that I care about elsewhere, and that I had already made plans to keep in contact with people outside of the site almost a year ago. But not everyone has made those sorts of preparations and overnight could lose years if not two decades' worth of personal interactions, connections, and shared content if something similar happens to them. Even just a major glitch like the one that [MySpace experienced][myspace-glitch] could nuke all those connections if you haven't built up resilient alternatives.

If there is no other takeaway from my experience here, it's that you should never put all your eggs in one basket when it comes to any tech service and that you cannot rely on the good nature or professionalism of a cloud provider to not upend your digital life. Maintain multiple modes of contact with those people that are important to you. Backup your important photos to your own offline storage solution. And bleed Zuckerber dry wherever you can.


<!-- links -->
[no-kings]: https://www.nokings.org/
[fb-community-standards]: https://transparency.meta.com/policies/community-standards/
[fb-account-integrity]: https://transparency.meta.com/policies/community-standards/account-integrity/
[fb-athentic-identity]: https://transparency.meta.com/policies/community-standards/authentic-identity-representation/
[fb-cybersecurity]: https://transparency.meta.com/policies/community-standards/cybersecurity/
[fb-inauthentic-behavior]: https://transparency.meta.com/policies/community-standards/inauthentic-behavior/
[proton-alias]: https://proton.me/support/creating-aliases
[MySpace experienced]: https://www.bit-tech.net/news/tech/software/myspace-glitch-deletes-masses-of-data/1/

<!--images-->
[login-screen]:             /assets/post-images/facebook-finally-banned-me/login-screen.png
[help-center]:              /assets/post-images/facebook-finally-banned-me/help-center-login-loop.png
[community-standards-png]:  /assets/post-images/facebook-finally-banned-me/community-standards.png
[authentic-identity-png]:   /assets/post-images/facebook-finally-banned-me/authentic-identity.png

